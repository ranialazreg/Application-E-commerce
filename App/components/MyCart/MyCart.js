...
const MyCart = ({ navigation }) => {
  ...
  const retrieveProducts = () => {
    ...
    CommandeService.getClientCommandes(clientId)
      .then((response) => {
  ...
  const sumOfCommande = () => {
    const clientId = user?.user?.id;
    CommandeService.PriceOfCommande(clientId)
      .then((response) => {
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            <TouchableOpacity onPress={() => deleteProduits(data.idC)}>
  ...
  const deleteProduits = (idC) => {
    CommandeService.DeleteProductsFromCommandes(idC)
      .then((response) => {
        retrieveProducts();
      })
      .catch((e) => {
  ...
  const totalValue = total?.value + 7;
  const ConfirmerCommandes = () => {
    const clientId = user?.user?.id;
    CommandeService.confirmAllCommandeProduits(clientId)
      .then((response) => {
        retrieveProducts();
        sumOfCommande();
      })
  ...
  const handleSelect = (livraison) => {
    setSelectedOption(livraison?.id);
  };
  const RadioButtonGroupTailles = ({
    livraisons,
    selectedOption,
    onSelect,
  }) => {
      ...
      <View>
        {livraisons.map((livraison) => (
          ...
          >
            ...
            <Text style={{ marginLeft: 8 }}>{livraison?.nom}</Text>
          </TouchableOpacity>
        ))}
      </View>
    );
      ...
      <ScrollView>
        ...
        <View style={{ paddingHorizontal: 16 }}>
          {products?.length != 0 ? (
            ...
            >
              Pas de produit maintenant dans votre carte
            </Text>
          )}
        </View>
        <View>
